...
- **Oscillation** — either condition of the oscillation rule below fires on the
  attempt ledger
...
- **`status: BLOCKED` / `BUDGET_EXHAUSTED`** — the circuit-break checkpoint is
  ...
  (`sdd-orchestrate/references/fan-out.md` §3e). `BUDGET_EXHAUSTED` requires
  `budget_consumed` in the dispatched `Budget:` units (Step 3 §Budget
  exhaustion).
