...
block when it fires, after the counters; (6b) implement stage gate only, the
plan completion parse — `PLAN: INCOMPLETE (N of M ticked)`, pausing with
`replan │ stop` **only** and suppressing (6)'s options; (7) the `TELEMETRY:` line, last, before
...
`loop-control.md` §5; (8b) implement stage gate `proceed` only, after (8): the
orchestrator — the sole writer of the plan's `status:` under orchestration —
flips `docs/ws/<id>/plan.md` to `status: complete` in its own bookkeeping
commit, editing `status:` only and leaving `research_id:` untouched.
