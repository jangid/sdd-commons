...
`sdd-orchestrate` is a **driver**, not a tenth phase skill. It composes the
...
`docs/.sdd-version`; if it is behind marker `4`, offer `/sdd-migrate` once
(informational, never forcing —
[`references/v4-workstreams.md`](references/v4-workstreams.md) §Upgrade offer).
Then run `python3 tools/sdd-gc.py --report`, render one line (`GC: clean` or
`GC: F fail, W warn — run tools/sdd-gc.py --report`) and continue regardless:
...
(`status:` first) and `sdd-review` emits an own-line `VERDICT:` token. Parse
...
The pipeline subagent invokes `sdd-<stage>` and writes the stage's
...
the chunk-close layer, returns `CHUNK_VERDICT: PASS | FAIL`; `sdd-implement`
...
`sdd-review`. Everything else — your reasoning, the pipeline subagent's
...
pipeline. With `on` the pipeline carries `Red team: enabled` (`sdd-verify`
...
reached**); at or past the cap route into `sdd-replan` only on an explicit
...
`**Depends on**: Chunk N` field — never modify `sdd-implement`), one
...
run `python3 tools/sdd-gc.py --report [--workstream <id>]`, render its findings
