#!/usr/bin/env python3
"""gc — drift sweep for the SDD docs corpus (`docs/**`).

`tools/skill-lint.py` keeps `skills/` honest; this sibling sweeps `docs/`
...
Three sweep classes (rule ids in brackets):
  ...
  delegated  sweeps 1-5, obtained by INVOKING tools/skill-lint.py as a
             ...
             seen at orchestrate entry / DONE, the two cadence moments
...
Usage:
  tools/gc.py [--report] [--fast] [--workstream <id>] [--root <path>]
  tools/gc.py --fix <rule> [--workstream <id>] [--root <path>]
  tools/gc.py --self-test | --help
...
ARCHIVE_FIX = ("rename to {YYYY-MM-DD}-{reason}.md (date from last_updated or the commit date); "
               "`-replan-` is reserved for replan archives")
...
class Gc:
    ...
    def sweep_lint(self) -> None:
        ...
        if not re.match(r"^(OK|FAIL): ", summary) or proc.returncode not in (0, 1):
            self.flag(Path("tools/skill-lint.py"), None, "lint",
                      f"linter exited {proc.returncode} without a parseable summary",
                      "run tools/skill-lint.py directly and fix its error")
                ...
                for f, st in sorted(status.items()):
                    if f.name in traced and st != "Approved":
                        self.flag(f, None, "spec-approval",
                                  ...
                                  "approve the spec (specs) before implementing against it")
        elif plans:
            for f, st in sorted(status.items()):
                if st != "Approved":
                    self.flag(f, None, "spec-approval",
        ...
        for d in self.plan_history_dirs():
            for f in sorted(d.glob("*.md")):
                ...
                if not ARCHIVE_NAME_RE.match(f.name):
                    ...
                    self.flag(f, None, "plan-history-name",
                              "`-replan-` archive never mentions a replan (written by replan only)",
                              ARCHIVE_FIX)
...
sweep classes and rule ids
  delegated (tools/skill-lint.py, invoked as a subprocess; never copied):
      ...
      [kickoff-fields] fail kickoff date:/research_id: per workstream (linter at marker 3;
             gc at marker 4, seen at orchestrate entry and DONE)
  gc (scoped to docs/**):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if gc.lint_path() is None:
        print(f"error: linter missing — expected {root / 'tools' / 'skill-lint.py'}")
        return 2
    return gc.run()
