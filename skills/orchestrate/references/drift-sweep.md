...
```
run:     python3 tools/gc.py --report            # exit 0 | 1 | 2
...
```
run:     python3 tools/gc.py --report [--workstream <id>]   # marker 4: the completed workstream
...
|---|---|---|
| mechanical | `xlink-dead` (unique resolution), `index-requirements` row, `traceability-aggregate`, `plan-history-name` | `python3 tools/gc.py --fix <rule>`; the **operator** reviews the printed paths and commits the rewrite (REQ-HARN-024) — the driver never commits a `--fix` |
...
The optional pre-commit profile `python3 tools/gc.py --fast` (lint +
