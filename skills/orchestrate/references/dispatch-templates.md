...
Copy-ready templates the `orchestrate` driver fills in at dispatch time.
...
second executors of an existing layer, never `review`.
...
Precedence: where the {stage} skill tells you to scan for the next ID,
...
Task:
  1. Invoke the {stage} skill via the Skill tool and follow it.
...
Do not perform any stage other than {stage}.
```
...
- `{verify_red_only}` — present only for `stage = verify` **and** only when the
  ...
  `Red team: enabled`; `verify` Step 6 keys on it to write
  ...
  otherwise — standalone `verify` and a red-off cycle must see no slot, so
...
{upstream_path_line}      # e.g. "Upstream artifact: docs/requirements/index.md"
                          # OMIT this line for the research stage — review
...
(verified absent in the RS-005 dispatch; mirrors `review` Step 2)
...
verifier it is a **leaf** (`return-contract.md` §1) and **never `review`**:
