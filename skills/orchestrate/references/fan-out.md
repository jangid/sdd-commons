...
subagent is a **leaf** that runs `implement` on its chunk-group and cannot fan
...
`implement` (REQ-ORCH-001, REQ-ORCH-016):
...
Stage skill to invoke: implement
...
Worktree pin (HARD boundary):
  ...
  - Do NOT edit the shared plan or traceability files (docs/plan.md /
    ...
    implement says to mark tasks [x] or fill traceability columns
...
Leaf clause:
  - You are a LEAF subagent. Do NOT dispatch any sub-subagent and do NOT fan out
    further — run implement directly on your chunk-group. (You have no
...
Precedence: where implement tells you to scan for the next ID or choose an
output path, THESE dispatch instructions override it.

Task:
  1. Invoke implement via the Skill tool and follow it on your chunk-group.
...
Do not perform any stage other than implement.
```
...
branch — §0). The verifier is read-only and is never `review`; the single
...
2. **Guaranteed fallback — abort and redo by re-derivation.** If auto-resolve is not
   ...
   **Marker-4 anchor (§0):** under `docs/.sdd-version` == `4`, re-branch the redo
   ...
   `<branch>-redo`) to **re-run `implement`** for that chunk-group in the fresh
...
4. **Checkpoint application** for every leaf whose `RETURN.status` is
   ...
   and `open_questions` per the mapping table in `implement/SKILL.md`
   ...
   gate — never invent a task. `replan` Step 1 reads this note as the
...
6. **Leaf deviations → Q-IMPL filed by the orchestrator.** Leaves never write
   ...
   spec's `## Implementation Questions` in the `implement` Q-IMPL format
