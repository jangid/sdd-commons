...
description: >
  ...
  replan if critical failures are found.
---
...
`verify` is one of four verification layers in the SDD workflow. The others are: chunk-close (mechanical, in-session, per-chunk), XSPEC (structural, in-session, during specs), and review (semantic, out-of-session, at phase boundaries). verify is the holistic in-session pass at end of project. If you find this skill's scope crossing into another layer's territory, refer to that layer's skill or spec.

**Red team (adversarial second executor — `docs/spec/adversarial-verify.md`).** At the verify stage `orchestrate` may dispatch, opt-in and default off, a read-only RED TEAM leaf that re-executes **this layer's Steps 3–4** adversarially — picking the weakest acceptance criteria and constructing inputs that violate them. It is a second *executor* of this layer, exactly as the chunk verifier is of chunk-close: not a fifth layer, not `review`, and the four-layer list above is unchanged. Standalone `verify` is unaffected except for the `pending-red` rule in Step 6.
...
1. If no `docs/plan.md` → use `plan`
2. **Staleness check**: compare `last_updated` in `docs/requirements/index.md` and specs against `docs/plan.md`'s `last_updated` frontmatter (legacy plans without frontmatter: file modification date as fallback). If upstream artifacts are newer than the plan, the plan is stale → use `plan` to update before verifying
   - **Workstream-scoped (marker `4` only)**: `docs/.sdd-version` is the sole gate. Under marker `3` (or earlier) run the whole-plan compare above — flat `docs/plan.md` vs all specs/requirements — **unchanged**. Under marker `4` `verify` gains a **new** workstream-scoped branch (it had no scoped branch before): compare the active workstream's `docs/ws/<ws>/plan.md` / `docs/ws/<ws>/verification.md` **only** against the shared specs/requirements that workstream traces, using the **same live plan-walk** as `plan`/`implement` (walk `<ws>`'s tasks' `traces to` specs → each spec's `requires:` requirement IDs → those specs' and requirement category files' `last_updated`; task → spec `requires:` → requirement IDs → category-file dates). It must **not** report staleness from shared-input changes outside `<ws>`'s traced set. This reads **no traceability file** and adds no traceability schema column — the scope is derived live (REQ-WS-027). See `docs/spec/ws-staleness.md`
3. If `docs/plan.md` has incomplete tasks → use `implement`
...
`orchestrate`, never by this skill.
...
`Red team: enabled` (`orchestrate` sets it only when the operator chose
`red team: on`):
...
`replan` routes it back here; `orchestrate` resumes before the red
dispatch).
...
- **Critical issues** → recommend `replan` with the failure context. Under
