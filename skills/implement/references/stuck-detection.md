...
summary). Read on demand, the way `orchestrate`'s references are.
...
4. **Recommend**: invoke `replan` with the stuck context (standalone), or
...
`replan` Step 4 already defines ("mark blocked tasks — note why they're
...
`orchestrate/references/fan-out.md` §3e post-merge bookkeeping. Full
tracebacks are never stored; `replan` Step 1 reads this note as its stuck
