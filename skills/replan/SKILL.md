...
1. If `docs/verification.md` exists with `status: fail` → replan from verification failures
   - A `verification.md` with `status: pending-red` is **not** a verification failure — blue passed and the red-team verdict is pending (`docs/spec/adversarial-verify.md` §`status: pending-red`). Do not replan from it: route to `verify` (re-enter the verify stage)
...
6. The **checkpoint** — the blocked-task note under the stuck task in
   ...
   `unblocks:` lines, ≤ ~15 lines; written by `implement` Step 3, or
...
- Action: invoke `research` for a spike, then cascade
...
- **Level 2** → tell user "this needs a spec update" and either:
  ...
  - Recommend invoking `specs` for the affected spec
  - Then revise the plan
- **Level 3** → recommend invoking `requirements` to capture the change, then cascade
- **Level 4** → recommend invoking `research` for the unknown, then cascade
...
segment. `orchestrate` derives the replan re-entry count (`REPLAN_MAX`,
default 3) by counting archives matching
`^(\d{4}-\d{2}-\d{2})-(m\d+-)?replan-.*\.md$` dated ≥ the kickoff `date:`;
`plan`'s rewrite archives (`{date}-{reason}.md`) and milestone-complete
...
2. **Mark blocked tasks** — note why they're blocked and what unblocks them.
   This blocked-task note is the **checkpoint slot**: `implement` Step 3
...
7. **Carry `## Post-cycle Fixes` across verbatim** — the optional,
   ...
   it, never convert its lines into tasks or mark them done. `plan` does not
   create it and `implement` does not read it as tasks
...
- If implementation should resume → recommend `implement`
- If a spec needs updating first → recommend `specs`
- If research is needed → recommend `research`
- If re-verification is needed → recommend `verify`
...
- **Don't implement during replan**: produce the revised plan, not code. Implementation happens in `implement`
