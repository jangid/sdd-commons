...
> reviewed in this session, stop and ask the operator to invoke review
> in a fresh session.
>
> Confirm one of:
...
review is one of four verification layers in the SDD workflow. Each catches different failure modes:
...
- Walk aggregate acceptance criteria (verify's territory)
...
- **Redirect cross-layer findings**: if a finding belongs to chunk-close, XSPEC, or verify, flag it and name the responsible layer
