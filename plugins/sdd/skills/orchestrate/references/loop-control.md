# Loop Control — orchestrate reference
...
after the last chunk: dispatch the implement-stage review ONCE on the merged state
...
- **Checkpoint on redo exhaustion (REQ-HARN-008).** When `REDO_MAX` fires at
  ...
  `implement/SKILL.md` §Step 3, with trigger label `fix-cap`, and applies
  ...
  `fan-out.md` §3e.4). `replan` Step 1 reads it as the stuck state.
- **FAIL routing.** A FAIL routes **only** to a repair packet for a redo of the
  ...
  or to `replan`. The per-chunk gate never shows a review `VERDICT:`.
- **Review runs ONCE.** The implement-stage `review` and its stage gate run
...
round 1 `APPROVE`, then a pipeline re-dispatch of `verify` that regenerated
...
- **Checkpoint on fix-loop exhaustion at an implement chunk (REQ-HARN-008).**
  ...
  table in `implement/SKILL.md` §Step 3, trigger label `fix-cap`, and
...
stored; recompute it on every replan trigger, before routing into `replan`:
...
- Only `replan` writes archives carrying the `-replan-` segment
  (`{date}-replan-{reason}.md`; per milestone `{date}-m{N}-replan-{reason}.md`).
  `plan` rewrite archives (`{date}-{reason}.md`) and milestone-complete
...
- **Neither determinable** (no `date:`, kickoff uncommitted, or `research_id:`
  ...
  every `-replan-` archive listed, and routes into `replan` only on an
...
- When `count >= REPLAN_MAX`, surface the cap as a gate event (REQ-ORCH-017
  ...
  route into `replan` without an explicit operator decision.
...
2. **Dispatch** one **leaf** implement subagent per group, pinned to its
   worktree/branch, carrying that group's chunks; a leaf runs `implement`
...
re-branched from the updated `main`, a fresh leaf re-running `implement`.
...
8b. **post-decision, implement stage gate `proceed` only, after item 8**: the
   ...
   **own** bookkeeping commit, the same post-gate slot as aggregate
   ...
   §7). It edits `status:` only; `plan`'s `research_id:` stamp is
   ...
   never writes `status:`, `verify` never writes the plan, and a direct
   (unorchestrated) `implement` session keeps its own Step 6.4 flip
...
- **Replan trigger**: if a pipeline subagent triggers a replan (stuck detection,
  ...
  route back to an earlier stage via `replan`, consistent with the cyclic
