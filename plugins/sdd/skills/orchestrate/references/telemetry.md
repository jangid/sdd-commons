...
**This table is a rendering of the domain table in `tools/telemetry.py`**,
...
Only `orchestrate` writes the file. Sequence per dispatch:
...
- **Absence is the signal**: an operator who sees a gate carrying no `rec` line,
  ...
  **except at a session's first gate**, where no append has yet been attempted
  ...
  `tools/telemetry.py summarize`'s `records-vs-expected:` line (§7) is the
...
`tools/scope-check-selftest.py` scenario F7.
...
`tools/gc.py` (drift sweep) and `tools/telemetry.py` are out-of-loop
...
`n_before := 0 if absent` rule the writer and `eval.py` follow), never an
