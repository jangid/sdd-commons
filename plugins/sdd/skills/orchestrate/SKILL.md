...
Then run `python3 <plugin-dir>/tools/gc.py --report --root .` (`<plugin-dir>` = this plugin's own root, so the tool that ships with the suite runs — the same convention `verify/SKILL.md` uses, `two-root-linter.md` §8; `--root .` keeps the operator's repository the subject),
...
run `python3 <plugin-dir>/tools/gc.py --report --root . [--workstream <id>]`, render its findings
