# `orchestrate` — Operator Guide

This is the **human-facing** guide to the `orchestrate` driver. The companion
...
`orchestrate` runs the nine SDD phase skills (research → requirements →
...
- You only want to review one finished artifact → use `review`.
...
```bash
ln -sfn "$HOME/work/github/jangid/tools-skills-agents/skills/orchestrate" \
...
- Verify with `ls -l ~/.claude/skills/` — every phase-skill entry should be a symlink
...
> Use orchestrate to take "<your idea>" through a full SDD cycle.
...
orch > [verify pipeline (Red team: enabled) → scope check → RED TEAM leaf → review]
       Verify stage gate — pipeline #9 (verify), red #10, review #11
...
`/orchestrate` again. It reads the existing artifacts on disk, detects which
...
1. **Pipeline subagent** — a fresh subagent invokes the stage skill
   (`research`, `requirements`, …) and writes the normal SDD artifact(s).
...
- **Fan-out and verification re-anchor** — implement-stage fan-out branches from
  ...
  PR), and `verify` diffs against the workstream's branch point, not `main`
  HEAD.
...
3. **LOOP — research.** Pipeline subagent runs `research`, writes
...
because `review` forbids kickoff prompts as input. The reviewer reads the
...
| A stage triggers a replan | Stuck detection / spike invalidation / verify failure. | It surfaces at the gate as a replan event; you may route back via `replan`. |
...
- **Replan re-entry cap** — when a stage triggers a replan, the orchestrator
  ...
  routes into `replan` only on your explicit decision.
...
`replan` already reads (`**Blocked** (<date>, <trigger>): checkpoint`, with
...
With `on`, the verify pipeline carries `Red team: enabled` — `verify` then
...
`tools/gc.py` is a sweep, not a stage. It runs at exactly two moments:

- **Entry** (before the workstream picker / phase detection): one line —
  `GC: clean`, or `GC: F fail, W warn — run tools/gc.py --report`, or `GC:
...
- **DONE** (after verify passes review and you approve): the tool's findings
  ...
  `traceability-aggregate`, `plan-history-name`) → `python3 <plugin-dir>/tools/gc.py --root .
  ...
  cycle's `verification.md` `## Next Steps` (the one slot `verify` Step 6
...
routing: `references/drift-sweep.md`; the tool: `python3 <plugin-dir>/tools/gc.py --root . --help`.
...
- provisions one git worktree/branch per group and dispatches one **leaf**
  implement subagent per group (each runs `implement` and cannot fan out
  further),
...
offending group by re-running `implement` in a worktree re-branched from the
...
existing project, run `verify` directly.)
...
- [`references/drift-sweep.md`](references/drift-sweep.md) — when
  `tools/gc.py` runs, how `GC:` renders and where each finding goes
- `tools/telemetry.py`, `tools/gc.py`, `tools/eval.py` — the
