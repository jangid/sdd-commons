...
1. If no `docs/requirements/index.md` or status is `Draft` → use `requirements` first
2. **Staleness check**: compare `last_updated` in `docs/requirements/index.md` against `last_updated` in each `docs/spec/*.md` and `docs/plan.md` (if they exist). If requirements are newer than existing specs or plan, those downstream artifacts are **stale** — they were written against older requirements and need updating. Proceed to write/update specs regardless of their current status
   - **Workstream-scoped (marker `4` only)**: `docs/.sdd-version` is the sole gate. Under marker `3` (or earlier) run the compare above — requirements vs each spec **and** the flat `docs/plan.md` — **unchanged**. Under marker `4` specs are **shared** and plans are **per-workstream** (there are N `docs/ws/<id>/plan.md`, no flat monolith), so `specs` **stops treating the flat plan as a monolith**: check **only** requirements→spec staleness (the shared corpus — compare `docs/requirements/index.md` against each `docs/spec/*.md`) and **defer plan staleness** to `plan`'s per-workstream branch. Do **not** compare against a global `docs/plan.md` under marker `4` (REQ-WS-027). See `docs/spec/ws-staleness.md`
3. If `docs/spec/*.md` all have `status: Approved` **and are not stale** (per check 2) → use `plan`
4. If `docs/plan.md` exists with incomplete tasks **and is not stale** (per check 2) → use `implement`
...
3. Leave the **Verified** column to `verify` — its three legal values are
...
When all specs are approved, recommend `plan` as the next step.
