...
2. If `docs/spec/*.md` are missing or any has `status: Draft` → use `specs`
...
4. If `docs/plan.md` exists with incomplete tasks **and is not stale** (per check 3) → use `implement`
...
1. **planned**: `plan` creates the file. Tasks defined but not started.
2. **active**: `implement` sets when work begins on the milestone's first task.
3. **complete**: `implement` (or `verify`) sets when all tasks are done and verified.
...
tasks: `implement` does not read them as tasks and they do not re-open the
task list.)
...
When invoked for an existing plan (e.g., after `replan` triggers):
...
When approved, recommend `implement` as the next step.
