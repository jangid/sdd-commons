#!/usr/bin/env python3
"""eval — score orchestrated SDD runs from `.sdd/telemetry.jsonl`.
...
only the derivation table of `skills/orchestrate/references/telemetry.md`
§6 — no prose, no artifact other than `verification.md`'s `status`.

Usage:
  tools/eval.py [--file .sdd/telemetry.jsonl] [--verification docs/ws/<id>/verification.md]
                    [--status pass|fail|pending-red] [--run-status RESEARCH_ID=STATUS ...]
                    [--workstream ID] [--csv]
  tools/eval.py --self-test        # six-record fixture in a temp dir
...
def self_test() -> int:
    ...
    if failures:
        print("eval self-test FAILED:")
        for f in failures:
            print(f"  - {f}")
        return 1
    print("eval self-test OK (6 records, 9 fields, aggregate, empty/missing file, csv)")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    for item in args.run_status:
        if "=" not in item:
            print(f"eval: --run-status expects RESEARCH_ID=STATUS, got {item!r}", file=sys.stderr)
            return 2
        rid, st = item.split("=", 1)
        run_status[rid] = st
    try:
        ...
        print(f"eval: cannot read {args.file}: {exc}", file=sys.stderr)
        return 2
