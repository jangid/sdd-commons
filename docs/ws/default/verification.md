...
3. **Dogfooding probe 2 (write-scope false-positive rate, RS-008 Q5) — open.**
   ...
   **[Closed 2026-09-20 as a settled exclusion — harness-p6 verify gate.** The spec-file `ADVISORY` class is **by design**, not noise to be tuned away: `docs/spec/*.md` is advisory in the implement row precisely so a leaf's spec edit is visible at the gate without being a violation. Re-opening REQ-HARN-026 to suppress it would remove the signal rather than the noise. harness-p6 did narrow that requirement — its §Recorded v1 Limitations lost the falsified "skills never stash" sentence when git-state observation shipped (REQ-HARN-HARNESSP6-001). Declined, not postponed; re-open only if a measured cycle shows the ADVISORY class hiding a real violation.]**
