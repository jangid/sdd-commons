...
- adversarial-verify.md §Manual: **done 2026-09-18** — the red team was run on the toy's verify stage (restored from `.pilot-toy/sdd-eval-toy.bundle` at `4aced03`), closing the live half of REQ-REDB-HARNESSP2-001. Three red rounds against `docs/spec/recon.md`:
  - **Round 1** (`4aced03`) → `RED_VERDICT: BROKEN`, R6 on acceptance criterion 6: `math.isfinite()` on an int outside float range raised `OverflowError`, so the tolerance path raised where the pre-tolerance engine returned a result. Blue (`sdd-verify`) had walked all 9 criteria and reported 0 critical / 0 material, and the stage review returned `VERDICT: APPROVE`, both before red. Routed `fix` → toy commit `7d42156` (recorded as a deviation entry in the toy's own spec).
  - **Round 2** (`7d42156`) → `RED_VERDICT: BROKEN`, R1 on the same criterion by a different mechanism: an int outside float *precision* (`2**53+1` vs `9007199254740992.0`) coerced silently to a `0.0` difference. Orchestrator-verified against the pre-tolerance engine (`6387970~1` returns `[1]`), so it predates the R6 fix. Routed `fix` → toy commit `96c1546` (likewise a toy-spec deviation entry).
  - **Round 3** (`96c1546`) → `RED_VERDICT: HELD`: criterion 6 re-attacked with an exhaustive 26×26 product of pathological amounts and a 20,000-case identity fuzz — 0 divergences. Two advisory-only spec gaps reported as `HELD` with no `failures[]` entry (`tolerance=float("nan")`; `TypeError` from mixed-type ids). Gate flipped `pending-red → pass`; toy commit `fbebd05`.
  - No `Rn` was `accept`ed, so the `accept (record)` path into §Issues Found → Minor remains unexercised, as does the write-revert rule (REQ-REDB-HARNESSP2-003 — red left its worktree clean all three rounds). Fix-loop counter reached `iteration 2 of 3`; `REPLAN_MAX` untouched.
  - Findings from the run (including two that change the harness: content-hash write-scope snapshots, and pinning the `RETURN:` block in every leaf template) are accumulated in the gitignored `.pilot-toy/harness-p3-input.md` for the `harness-p3` cycle.
