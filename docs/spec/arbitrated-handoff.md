...
- [ ] The command below hits inside §`W_N` Includes Regeneration Writes and §2a respectively with matching wording; scenario A1 yields `class b` with two annotated keys under the diff-based rule and no token under provenance, printed side by side; no Q-IMPL-HARNESSP5-* entry is needed (the Approved sentence is the record); `python3 plugins/sdd/tools/skill-lint.py` exits 0 (REQ-ARB-HARNESSP5-001):

  ```
  grep -n 'byte-identical' docs/spec/arbitrated-handoff.md plugins/sdd/skills/orchestrate/references/loop-control.md
  ```
